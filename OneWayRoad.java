...
public class OneWayRoad extends Road {
	 ...
	 */	 
	public OneWayRoad(String identification, Location l1, Location l2, int length, float speedLimit,
			float averageSpeed) {
 ...
 */
@Override 
public void setIsBlocked(boolean isBlocked,boolean direction) throws IllegalArgumentException {
	if(direction == false) {
		throw new IllegalArgumentException();
	}
	else {
 ...
 */
@Override
public boolean isTraversable(boolean direction) throws IllegalArgumentException{
	if(direction == false) {
		throw new IllegalArgumentException();
	}
if(getIsBlocked(true)) {
